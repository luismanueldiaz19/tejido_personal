...
class PrintMainForPaid {
  ...
  static Future<File> generate(List<ForPaid> listPlani, String fecha) async {
    ...
    pdf.addPage(MultiPage(
      build: (context) => [
        buildPaymentInfoRowImagen('Lista de cuentas', imageLogo2),
          ...
          child: Row(
            ...
            children: [
              ...
              Container(
                  ...
                  child: Center(
                      child: Text(
                          'Vencidos : ${getNumFormatedDouble(ForPaid.getTotalVencidos(listPlani))}',
                          style: style))),
              SizedBox(width: 10),
              Container(
                  height: 25,
                  child: Center(
                      child: Text(
                          'Percent : ${ForPaid.getPercent(ForPaid.getTotalVencidos(listPlani), listPlani.length.toString())} %',
                          style: style))),
              SizedBox(width: 10),
              Container(
                  height: 25,
              ...
              Container(
                  ...
                  child: Center(
                      child: Text(
                          'Pagado : ${getNumFormatedDouble(listPlani.first.statuPaid == 't' ? '0' : ForPaid.getTotalRestate(listPlani))}',
                          style: style))),
              SizedBox(width: 10),
              Container(
                  height: 25,
                  child: Center(
                      child: Text(
                          'RD\$ Pagado : ${ForPaid.getPercent(listPlani.first.statuPaid == 't' ? '0' : ForPaid.getTotalRestate(listPlani), ForPaid.getTotalMonto(listPlani))} %',
                          style: style))),
              SizedBox(width: 10),
              Container(
                  height: 25,
                  child: Center(
                      child: Text(
                          'Balance : ${getNumFormatedDouble(listPlani.first.statuPaid == 't' ? ForPaid.getTotalMonto(listPlani) : ForPaid.getTotalBalance(listPlani))}',
                          style: style))),
            ],
          ),
        ),
  ...
  static Widget tableProducts(List<ForPaid> listProduct) {
    ...
    final data = listProduct
        .map((producto) => [
              ...
              producto.numInter.toString(),
              producto.dias.toString(),
              producto.fFecha.toString(),
              producto.fFechaVencimiento.toString(),
              producto.aproved.toString(),
            ])
        .toList();
    return Table.fromTextArray(
      ...
      headerStyle: const TextStyle(fontSize: 4),
