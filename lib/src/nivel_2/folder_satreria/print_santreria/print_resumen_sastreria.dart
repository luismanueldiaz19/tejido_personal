...
class PdfResumeSastreria {
  ...
  static Future<File> generate(
    ...
    pdf.addPage(MultiPage(
      build: (context) => [
        buildPaymentInfoRowImagen(current?.nameDepartment ?? 'N/A', image),
  ...
  static Widget tableProducts(List<Confeccion> listProduct) {
    final headers = ['TIPO TRABAJOS', 'CANT PIEZA'];

    final data = listProduct.map((item) {
      return [item.tipoTrabajo, item.cantPieza];
    }).toList();
