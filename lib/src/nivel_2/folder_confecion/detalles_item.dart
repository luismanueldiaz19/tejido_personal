...
class DetallesItem extends StatefulWidget {
  const DetallesItem({Key? key, this.item}) : super(key: key);
  final Confeccion? item;

  @override
  State<DetallesItem> createState() => _DetallesItemState();
}

class _DetallesItemState extends State<DetallesItem> {
...
//---------------------------------------------//
  ...
  Future updateWorkConfecion() async {
    if (controllerLogo.text.isNotEmpty &&
      ...
      await httpRequestDatabase(updateReportConfeccion, data);
      //  print(res.body);
  ...
  Future reportMake() async {
    if (cant.text.isNotEmpty) {
      ...
      await httpRequestDatabase(updateReportConfeccionCantPieza, data);
      // print(res.body);
      if (mounted) Navigator.pop(context, true);
    } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Column(
        children: [
          const AppBarCustom(title: 'Detalles Confeción'),
          isUpdatedModo
              ? Expanded(
