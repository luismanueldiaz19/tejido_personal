...
class PdfApisFinalOrden {
  static Future<File> generate(Invoice invoice) async {
    ...
    return PdfApi.saveDocument(name: 'OrdenesFinal.pdf', pdf: pdf);
  }
  ...
  static Widget buildInvoiceInfo(InvoiceInfo info) {
    ...
    final data = <String>[info.date.toString().substring(0, 10)];
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
  ...
  static Widget buildTitle(Invoice invoice) => Column(
        ...
        children: [
          ...
          Text('${invoice.itemsFinalOrder?[0].dateCurrent}'),
  ...
  static Widget buildInvoice(Invoice invoice) {
    ...
    final data = invoice.itemsFinalOrder?.map((item) {
      return [
        item.numOrden,
        item.logo,
        item.calida == 't' ? 'Verificada' : 'N/A',
        item.cantPcs,
        item.pcsTirada,
        item.pcsBad,
        item.cantTirada,
